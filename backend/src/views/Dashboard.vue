  ...
  <div class="grid grid-rows-1 md:grid-rows-2 md:grid-flow-col grid-cols-1 md:grid-cols-3 gap-3">
    ...
    <div class="bg-white py-6 px-5 rounded-lg shadow">
      ...
      <template v-if="!loading.latestCustomers">
        <router-link :to="{name: 'app.customers.view', params: {id: c.id, user_id: c.id}}" v-for="c of latestCustomers" :key="c.id"
                     class="mb-3 flex">
...
function generateRandomCountriesData() {
  // Generate random data for countries
  const countries = [
    { name: "Australia", count: Math.floor(Math.random() * 100) },
    { name: "USA", count: Math.floor(Math.random() * 100) },
    { name: "Canada", count: Math.floor(Math.random() * 100) },
    { name: "India", count: Math.floor(Math.random() * 100) },
  ];
  ...
  // Populate chart data with the generated countries data
